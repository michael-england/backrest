...
Collection.prototype.group = function(options, callback) {
    if (options.keyf != undefined && (typeof options.keyf) == "string") {
        eval("options.keyf = " + options.keyf);
    }
    
    if (options.finalize != undefined && (typeof options.finalize) == "string") {
        eval("options.finalize = " + options.finalize);
    }
    
    this._exec('group', [options.key || (options.keyf || {}), options.cond || {}, options.initial || {}, options.reduce, options.finalize, options.command, callback]);
};

Collection.prototype.mapReduce = function(options, callback) {
    ...
    if (options.reduce != undefined) {
        if ((typeof options.reduce) == "string") {
    ...
    if (options.finalize != undefined && (typeof options.finalize) == "string") {
        eval("options.finalize = " + options.reduce);
    }
    
    this._exec('mapReduce', [map, reduce, options, callback]);
};
