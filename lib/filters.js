				        ...
				        if (params instanceof Array) {
				            for (var n = 0; n < params.length; n++) {
				                if (params[n][filters[i].fieldToFilter] != undefined && 
				                    params[n][filters[i].fieldToFilter] != null) {
				                    params[n][filters[i].fieldToFilter] = undefined;
				                }
				            }
				        } else {
	    ...
	    if (settings.collections != undefined) {
	        if (settings.collections[collection] != undefined) {
	            if (settings.collections[collection][method] != undefined) {
	                if (settings.collections[collection][method].filters != undefined) {
	                    filters = settings.collections[collection][method].filters;
	                }
	            }
	        }
	    }
