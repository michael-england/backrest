            ...
            if (collection) {
                if (server.settings.collections[collection]) {
                    ...
                    if (json.jsonrpc === "2.0") {
                        ...
                        if (collection == server.settings.authentication.collection && authenticationMethods.indexOf(json.method) > -1) {

                            // process the authenciation method
                            this[json.method](server, request, response, json.params);
                        } else {
                            next();
                        }
                    } else {
...
module.exports.login = function(server, request, response, params) {

    var isValid = true;

    // filter the params
    params = filters.filter(server.settings, server.settings.authentication.collection, "login", "default", params, "in");

    // validate
    validators.validate(server, request, server.settings.authentication.collection, "login", "default", params, function(validationSummary) {
        ...
        if (isValid) {
            ...
            var dbLoginResult = function(error, result) {
                if (error) {
                    ...
                    if (!result) {
                        ...
                        // collection not provided, create procedure not found response
                        server.error(request, response, -32000, "Invalid credentials.");
                        ...
                        if (passwordHash.verify(password, result[passwordField])) {
                            ...
                            if (emailConfirmed) {
                                ...
                                // return result
                                server.result(request, response, resultFiltered);
                            } else {

                                // collection not provided, create procedure not found response
                                server.error(request, response, -32000, "Email not confirmed.");
                            }
            ...
            console.log(request.sessionID + ": server.db." + server.settings.authentication.collection + ".findOne(" + JSON.stringify(params) + ", dbLoginResult);");

            // execute command
            server.db[server.settings.authentication.collection].findOne(params, dbLoginResult.bind(this));
        } else {
...
module.exports.logout = function(server, request, response, params) {
    ...
    // return result
    server.result(request, response, "Logout successful.");
    return;
};
...
module.exports.passwordReset = function(server, request, response, params) {
    ...
    // validate
    validators.validate(server, request, server.settings.authentication.collection, "passwordReset", "default", params, function(validationSummary) {
        if (validationSummary !== true) {
            isValid = false;
        }
...
module.exports.isAuthenticated = function(server, request, response, params) {
    ...
    // return result
    server.result(request, response, isAuthenticated);
    return;
};
