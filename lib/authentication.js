...
exports.login = function(server, request, response, json) {
    ...
    validators.validate(server, request, server.settings.authentication.collection, "login", "default", json, json.params, function(validationSummary) {
        ...
        if (isValid) {
                    ...
                    if (!result) {
                        ...
                        if (passwordHash.verify(password, result[passwordField])) {
                            ...
                            if (emailConfirmed) {
                                ...
                                console.log("Session " + request.session.id + " is now logged in as " + result[(!server.settings.authentication.usernameField ? "email" : server.settings.authentication.usernameField)]);
            ...
            console.log(request.session.id + ": server.db." + server.settings.authentication.collection + ".findOne(" + JSON.stringify(json.params) + ", dbLoginResult);");
            ...
            if (isValid) {
                ...
                if (passwordHash.verify(password, request.session.data.user[passwordField])) {
                    ...
                    if (json.params.newPassword == json.params.confirmPassword) {
                        ...
                        // write command to log
                        console.log(request.session.id + ": server.db." + server.settings.authentication.collection + ".update({\"_id\":server.db.ObjectId(\"" + request.session.data.user._id.toString() + "\")}," + JSON.stringify(update) + ", dbResult);");

                        // execute command
                        server.db[server.settings.authentication.collection].update({"_id":request.session.data.user._id}, update, dbResult.bind(this));
                    } else {
...
exports.passwordResetRequest = function(server, request, response, json) {
    ...
    validators.validate(server, request, server.settings.authentication.collection, "passwordResetRequest", "default", json, json.params, function(validationSummary) {
        ...
        if (isValid) {
            ...
            // write command to log
            console.log(request.session.id + ": server.db." + server.settings.authentication.collection + ".findOne(" + JSON.stringify(json.params) + ", dbResult);");
...
exports.isInRole = function(server, request, response, json) {
    ...
    if (request.session.data.user != "guest" && json.params !== undefined && json.params !== null) {
        if (json.params.name !== undefined) {
            var roles = request.session.data.user[server.settings.authentication.rolesField !== undefined ? server.settings.authentication.rolesField : "roles"];
            if (roles !== undefined) {
