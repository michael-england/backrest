                ...
                if (server.settings.collections[collection] || uri.pathname.replace("/", "") === "_settings") {
                    ...
                    if (json.jsonrpc === "2.0") {
                        ...
                        if (collection === "_settings" && server.settings.isDebug && json.method === "get") {
                            ...
                            server.result(request, response, server.settings, json.id);

                        } else {

                            // process jsonrpc request
                            this.process(server, request, response, json, collection);
                        }
                    } else {
                        next();
                    }
...
module.exports.process = function(server, request, response, json, collection) {
    ...
    try {
        ...
        if (method instanceof Array) {
            ...
            var processMethod = function() {
                ...
                // check roles
                roles.check(server, collection, method[index], action, request.session.user, json.params[index], function(allowed) {
                    if (allowed) {
            ...
            // check roles
            roles.check(server, collection, method, action, request.session.user, json.params, function(allowed) {
                if (allowed) {
                    if (method === "update") {
...
module.exports.dbResult = function(server, request, response, collection, method, action, json, error, result) {
...
module.exports.update = function(server, request, response, collection, method, action, json) {
    ...
    var validationComplete = function(validationSummaryFind, validationSummaryUpdate) {
        if (validationSummaryFind === true && validationSummaryUpdate === true) {
            ...
            // write command to log
            console.log(request.sessionID + ": server.db." + collection + "." + method + "(" + JSON.stringify(json.params[0]) + "," + JSON.stringify(json.params[1]) + ", dbResult);");
                ...
                validators.validate(server, request, collection, "update", action, json, json.params[1][keys[0]], function(validationSummaryUpdate) {
                    ...
                    if (validationSummaryUpdate === true) {
                        ...
                        if (json.params[0]._id !== undefined) {
                            ...
                            // save uploads
                            json.params[1][keys[0]] = server.uploads.save(collection, id, request.session.uploads, request, json.params[1][keys[0]]);
                ...
                validators.validate(server, request, collection, "update", action, json, json.params[1], function(validationSummaryUpdate) {
                    ...
                    if (validationSummaryUpdate === true) {
                        ...
                        if (json.params[0]._id !== undefined) {
                            ...
                            // save uploads
                            json.params[1] = server.uploads.save(server, collection, id, request.session.uploads, request, json.params[1]);
...
module.exports.findAndModify = function(server, request, response, collection, method, action, json) {
    ...
    var validationComplete = function(validationSummaryFind, validationSummaryFindAndModify) {
        if (validationSummaryFind === true && validationSummaryFindAndModify === true) {
            ...
            // write command to log
            console.log(request.sessionID + ": server.db." + collection + "." + method + "(" + JSON.stringify(json.params) + ", dbResult);");
                ...
                validators.validate(server, request, collection, "findAndModify", action, json, json.params.update[keys[0]], function(validationSummaryFindAndModify) {
                    ...
                    if (validationSummaryFindAndModify === true) {
                        ...
                        if (json.params.query._id) {
                            ...
                            // save uploads
                            json.params.update[keys[0]] = server.uploads.save(server, collection, id, request.session.uploads, request, json.params.update[keys[0]]);
                ...
                validators.validate(server, request, collection, "findAndModify", action, json, json.params.update, function(validationSummaryFindAndModify) {
                    ...
                    if (validationSummaryFindAndModify === true) {
                        ...
                        if (json.params.query._id !== undefined) {
                            ...
                            // save uploads
                            json.params.update = server.uploads.save(collection, id, request.session.uploads, request, json.update);
...
module.exports.group = function(server, request, response, collection, method, action, json) {
...
module.exports.mapReduce = function(server, request, response, collection, method, action, json) {
    ...
    var validationComplete = function(validationSummary) {
        if (validationSummary === true) {
            ...
            // write command to log
            console.log(request.sessionID + ": server.db." + collection + "." + method + "(" + JSON.stringify(json.params[0]) + "," + JSON.stringify(json.params[1]) + "," + JSON.stringify(json.params[2]) + ", dbResult);");
...
module.exports.method = function(server, request, response, collection, method, action, json) {
...
module.exports.ObjectId = function (server, request, response, json, params) {
    if (params) {
        if (params._id) {
