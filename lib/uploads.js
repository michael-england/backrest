(function() {
    ...
    return function(request, response, next) {
      if (request.headers['content-type'] && request.headers['content-type'].indexOf('multipart/form-data') > -1) {
        ...
        if (request.body.method === 'upload') {
          ...
          // upload the files
          this.upload(server, request, response, request.body, request.files);
        } else {
          next();
        }
        ...
        if (collection) {
          if (server.settings.collections[collection]) {
            ...
            if (json.jsonrpc === '2.0' && json.method === 'clearUpload') {

              // process the clear uploads request
              this.clearUpload(server, request, response, collection, json.params);
            } else {
              next();
            }
          } else {
  ...
  module.exports.upload = function(server, request, response, fields, files) {

    // retrieve the field name of the file being uploaded
    var field = Object.keys(files)[0];
  ...
  module.exports.render = function(server, request, response, collection, field, step, _id, origin, temp) {
  ...
  module.exports.clearUpload = function(server, request, response, collection, params) {
    if (request.session.uploads !== undefined && params._id !== undefined && params.field !== undefined) {
      if (request.session.uploads[collection] !== undefined) {
        if (request.session.uploads[collection][params._id] !== undefined) {
          if (request.session.uploads[collection][params._id][params.field] !== undefined) {

            // write to log
            console.log(request.sessionID + ': removed an uploaded file.');

            // remove the upload
            delete request.session.uploads[collection][params._id][params.field];
  ...
  module.exports.resize = function(server, request, response, fields, field, path, sizes, index) {
          ...
          imagemagick.resize(options, function(error) {
            if (error) {
              ...
              if (server.settings.isDebug) {
                ...
                // email error
                server.sendErrorEmail(request, undefined, error, function() {
                ...
                // email error
                server.sendErrorEmail(request, undefined, error);
                return;
              }
            } else {
  ...
  module.exports.get = function(collection, _id, uploads) {
    var files;
  ...
  module.exports.has = function(collection, _id, uploads) {
  ...
  module.exports.save = function(server, collection, _id, uploads, request, params, isNew) {
  ...
  module.exports.clear = function(collection, _id, uploads) {
    if (uploads !== undefined) {
      if (uploads[collection] !== undefined) {
        if (uploads[collection][_id] !== undefined) {
  ...
  module.exports.error = function(error, request, response, next) {
