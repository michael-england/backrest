exports.validate = function(server, request, collection, method, action, json, params, callback) {
            ...
            switch (validators[index].type) {
                case "required":
                    if (value === "" || value === undefined) {
                        if (validators[index].errorMessage !== "" && validators[index].errorMessage !== undefined) {
                ...
                case "custom":
                    if (server.customValidators !== undefined && validators[index].validationFunction !== undefined && validators[index].validationFunction !== "") {
                        if (server.customValidators[validators[index].validationFunction] !== undefined) {
                            server.customValidators[validators[index].validationFunction](server, request, value, json, function(isValid) {
                                if (!isValid) {
                                    if (validators[index].errorMessage !== "" && validators[index].errorMessage !== undefined) {
                                        validationSummary.push(validators[index].errorMessage);
                                    } else {
